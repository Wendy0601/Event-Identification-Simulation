...
%
% clear all
function [bus_v,bus_freq,theta,mac_ang,mac_spd,flag_err,ilf,V1,V2,bus_sol]=s_simulwt(sw_con1, load_con1,lmod_con1,bus1,line1)
clear global
...
y_switch_lwt % calculates the reduced y matrices for the different switching conditions
...
if ~isempty(dcsp_con)
   ...
   Vr2 = V_rncprf;
   bo = boprf;
   [h_sol,flag_err] = i_simu_lwt(1,1,k_inc,h,bus_sim,Y1,Y2,Y3,Y4,Vr1,Vr2,bo);
...
while (kt<=ktmax)
   ...
   for k = k_start:k_end
      ...
      if k >= sum(k_inc(1:3))+1
         ...
         Vr2 = V_rncpf2;
         bo = bopf2;
         [h_sol,flag_err] = i_simu_lwt(k,ks,k_inc,h,bus_sim,Y1,Y2,Y3,Y4,Vr1,Vr2,bo);     
...
% 	     xlabel('time (s)');
      ...
      if j >= sum(k_inc(1:3))+1
         ...
         Vr2 = V_rncpf2;
         bo = bopf2;
         [h_sol,flag_err] = i_simu_lwt(j,ks,k_inc,h,bus_sim,Y1,Y2,Y3,Y4,Vr1,Vr2,bo);     
