...
if f_type == 7;
   ...
   f_nearbus = sw_con(2,2);
   bus_idx = find(bus_f(:,1)==f_nearbus);
   line_idx = find((line_f(:,1)==f_nearbus)|(line_f(:,2)==f_nearbus));
   if isempty(bus_idx)
      error('can not find faulted bus in bus data')
   end
   new_bus = max(bus(:,1))+10; % add new faulted bus
   max_f = length(bus(:,1))+1;
   bus_f(max_f,1) = new_bus;
   bus_f(max_f,2) = bus_f(bus_idx(1),2);;
   bus_f(max_f,3)=-1;%voltage anglebus(bus_idx(1),3);
   bus_f(max_f,4)=0;
   bus_f(max_f,5)=0; %generate   pu reactive power
   bus_f(max_f,6)=0;
   bus_f(max_f,7)=-0.2; % consume reactive power
   bus_f(max_f,8:9) = bus_f(bus_idx(1),8:9);
   bus_f(max_f,10) = 3;  %2:reactive generator 3: load 
   bus_f(max_f,11:15) = bus_f(bus_idx(1),11:15);
   df = length(line_f(:,1))+1; % add new line
   line_f(df,1)=new_bus;
   line_f(df,2)=f_nearbus;
   line_f(df,3) = line(line_idx(1),3); 
   line_f(df,4) = line(line_idx(1),4); 
   line_f(df,5:10) = line(line_idx(1),5:10); 
end
...
if f_type<4
   ...
   % admittance matrix
   bus_intpf1 = bus_int;
elseif f_type==4|f_type==5|f_type==6 | f_type==7;
